...
@section('content')
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('meetings.update', ['id' => $meeting->id]) }}" class="form-submit-event"
                    method="POST">
                    <input type="hidden" name="redirect_url" value="{{ route('meetings.index') }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="text" id="title" name="title"
                                placeholder="Enter Title" value="{{ $meeting->title }}">
                            @error('title')
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-4">
                            <label class="form-label" for=""><?= get_label('starts_at', 'Starts at') ?> <span
                                    class="asterisk">*</span></label>
                            <input type="text" id="start_date" name="start_date" class="form-control"
                                value="{{ format_date($meeting->start_date_time) }}">
                            @error('start_date')
                                <p class="text-danger text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-3 col-md-2">
                            ...
                            <input type="time" name="start_time" class="form-control"
                                value="{{ format_date($meeting->start_date_time, null, 'H:i:s') }}">
                            @error('start_time')
                                <p class="text-danger text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-3 col-md-4">
                            ...
                            <input type="text" id="end_date" name="end_date" class="form-control"
                                value="{{ format_date($meeting->end_date_time) }}">
                            @error('end_date')
                                <p class="text-danger text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="mb-3 col-md-2">
                            ...
                            <input type="time" name="end_time" class="form-control"
                                value="{{ format_date($meeting->end_date_time, null, 'H:i:s') }}">
                            @error('end_time')
                                <p class="text-danger text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
