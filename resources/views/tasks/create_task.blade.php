...
@section('content')
    <div class="container-fluid">
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1">
                        ...
                        <li class="breadcrumb-item">
                            <a
                                href="{{ url(isset($project->id) ? route('projects.tasks.draggable', ['id' => $project->id]) : route('tasks.index')) }}"><?= get_label('tasks', 'Tasks') ?></a>
                        </li>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('tasks.store') }}" class="form-submit-event" method="POST">
                    @if (isset($project->id))
                        <input type="hidden" name="project" value="{{ $project->id }}">
                        <input type="hidden" name="redirect_url"
                            value="{{ route('projects.tasks.draggable', ['id' => $project->id]) }}">
                    @else
                    ...
                    ?>
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <select id="" class="form-control js-example-basic-multiple" name="project"
                                    data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
                                    <option value=""></option>
                                    @foreach ($projects as $project)
                                        <option value="{{ $project->id }}"
                                            {{ old('project') == $project->id ? 'selected' : '' }}>{{ $project->title }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
